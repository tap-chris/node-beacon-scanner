...
* Method: startScan()
* ---------------------------------------------------------------- */
BeaconScanner.prototype.startScan = function (serviceUUIDs = [], allowDuplicates = false) {
	let promise = new Promise((resolve, reject) => {
		this._init().then(() => {
			this._prepareScan(serviceUUIDs, allowDuplicates);
		}).then(() => {
			resolve();
		}).catch((error) => {
			reject(error);
...
BeaconScanner.prototype._prepareScan = function (serviceUUIDs, allowDuplicates) {
	let promise = new Promise((resolve, reject) => {
		this.noble.startScanning(serviceUUIDs, allowDuplicates, (error) => {
			if (error) {
				reject(error);
			} else {
